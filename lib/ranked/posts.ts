import { getRankedContentDetail, getThisSiteProjectId, isRankedConfigured, listRankedContent } from './client'
...
function significantWords(title: string): Set<string> {
  const stop = new Set([
    'the', 'a', 'an', 'in', 'of', 'for', 'to', 'and', 'or', 'your', 'with', 'after',
    'when', 'what', 'that', 'from', 'how', 'into', 'without', 'about', 'this',
    'these', 'those', 'over', 'under', 'near', 'into', 'care',
  ])
  const places = new Set(['rio', 'rancho', 'bernalillo', 'albuquerque', 'family', 'families', 'active'])
  return new Set(
    title
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, ' ')
      .split(/\s+/)
      .filter((word) => word.length > 3 && !stop.has(word) && !places.has(word))
      .map((word) => word.replace(/s$/, '')),
  )
}

function isNearDuplicateTitle(title: string, existing: string[]): boolean {
  const left = significantWords(title)
  if (left.size === 0) return false
  for (const other of existing) {
    const right = significantWords(other)
    ...
    if (overlap >= 2 && ratio >= 0.45) return true
  }
  return false
}
  ...
  try {
    ...
    const resolved = await Promise.all(
      candidates.map(async (item) => {
        if (isNearDuplicateTitle(item.title, localTitles)) {
          console.info(`[ranked] skipping duplicate of a compiled post: ${item.title}`)
          return null
        }
